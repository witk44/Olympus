...
from resources.code.entity import Entity
...
class Player(Entity):
    def __init__(self,pos,groups,obstacle_sprites,attackable_sprites,create_attack,destroy_attack,create_magic):
        ...
        if os.path.exists(app_path("resources/save_files/player.json")):
            saved_player_file = open(app_path("resources/save_files/player.json"))
            saved_player = json.load(saved_player_file)
            self.sprite_type = saved_player["sprite_type"]
            self.image = pygame.image.load(app_path('resources/graphics/player/' + self.sprite_type + '.png')).convert_alpha()
            ...
            self.weapon_attack_sound = pygame.mixer.Sound(app_path('resources/audio/sword.wav'))
            self.weapon_attack_sound.set_volume(0.3)
        else:
            self.sprite_type = 'player'
            self.image = pygame.image.load(app_path('resources/graphics/player/player.png')).convert_alpha()
    ...
    def import_player_assets(self):
        character_path = app_path('resources/graphics/player/')
    ...
    def input(self):
        if not self.attacking:
            ...
            if keys[pygame.K_ESCAPE] :
                s = self.serialize()
                out_file = open(app_path("resources/save_files/player.json"), "w")
